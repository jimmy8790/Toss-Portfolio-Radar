...
- 포트폴리오 snapshot을 이용한 총 평가금액, 총수익률, 평균 하락위험점수 변화 그래프
- 미국 주식의 USD 평가금액과 KRW 환산 평가금액 표시
- 미국 주식의 USD 평가금액, KRW 환산 평가금액, 비용 포함 API 총수익률 비교 표시
- 원화 총수익률 입력 시 추정 매수환율과 환율 효과 계산
- 보유 종목별 수수료와 세금 표시
- 해외주식 종목별 원화 원금/원화 수익률은 현재 OpenAPI item 응답에 없어 자동 조회 불가 안내 표시
...
- `포트폴리오`: 보유 종목, 요약 카드, 해외주식 환율 참고, 종목 상세 분석
