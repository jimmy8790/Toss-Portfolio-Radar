...
def holdings_to_dataframe(holdings: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    df["profitLoss.rate"] = _pick_column(normalized, ["profitLoss.rate", "profitRate", "returnRate"])
    df["dailyProfitLoss.rate"] = _pick_column(normalized, ["dailyProfitLoss.rate", "dailyProfitRate"])
    df["evaluationAmount"] = _pick_column(
        normalized,
        ["evaluationAmount", "marketValue.amount", "marketValue.amountAfterCost", "marketValue", "value"],
    )
    ...
    df["profitLoss.amount"] = _pick_column(normalized, ["profitLoss.amount", "profitLossAmount"])
...
def calculate_summary(holdings_df: pd.DataFrame, usd_krw: float | None = None) -> dict[str, float | int | None]:
